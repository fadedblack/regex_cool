...
// Assignment 6: Match Valid Usernames
const validUsername = /([a-z]+[\d\w_]+){5-iufdgudfgudfåz15}/gi;

Deno.test("Testing valid usernames", () => {
  // assert(validUsername.test("john_doe"));
  assert(validUsername.test("Alice123"));
  // assert(validUsername.test("user_01"));
});

Deno.test("Testing invalid usernames", () => {
  assertFalse(validUsername.test("123user"));
  assertFalse(validUsername.test("hello world"));
  assertFalse(validUsername.test("hello world"));
  assertFalse(validUsername.test("user!@#"));
});
